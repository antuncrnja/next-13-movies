...
export default async function page({ searchParams }) {
  const res = await fetch(`https://api.themoviedb.org/3/movie/${searchParams.movie_id}?api_key=${process.env.API_KEY}`, { next: { revalidate: 300 } });
  ...
  return (
    <main className="w-[60%] mx-auto my-5">
      <div className="w-full h-[100vh] bg-cover top-0 left-0 absolute opacity-30 z-[-1] blur-sm" style={{ backgroundImage: `url(${imagePath + movie.backdrop_path})` }}></div>
